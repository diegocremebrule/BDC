...
public class G17HW2 {
    ...
    public static Vector[] MRFairLloyd(JavaPairRDD<Vector,Integer> U, int K, int M) {
        ...
        for (int i = 0; i < M; i++) { // we start for loop until M times
            ...
            JavaPairRDD<Integer, Tuple2<Vector, Integer>> clusteredPoints = U.mapToPair(coord_label -> {
                Vector coords = coord_label._1;  // get the coordinates of the points
                Integer label = coord_label._2;  // get the label (A/B), represented by (0/1)
                int idx = idx_close(coords, finalCentroids); // get the index of the centroid which is closest to coordinates
                return new Tuple2<>(idx, new Tuple2<>(coords,label)); //returning this (idx, (coordinates, label))
            });
            ...
            Map<Integer, Iterable<Tuple2<Vector, Integer>>> grouped = clusteredPoints.groupByKey().collectAsMap();

            //lists A and B for each cluster (k clusters total)
            List<List<Vector>> A = new ArrayList<>(K);
            List<List<Vector>> B = new ArrayList<>(K);
            for (int j = 0; j < K; j++) {
                A.add(new ArrayList<>()); //empty lists so cluster goes in position i of A and B respectively
                B.add(new ArrayList<>());

                //important!! this gets the list of all data points assigned to cluster i
                Iterable<Tuple2<Vector, Integer>> items = grouped.getOrDefault(j, new ArrayList<>());
                ...
                for (Tuple2<Vector, Integer> pointLabel: items) {
                    Vector vec = pointLabel._1;
                    int label = pointLabel._2;
                    if (label==0) {
                        A.get(j).add(vec);
                    } else {
                        B.get(j).add(vec);
    ...
    public static Result computeVectors(List<List<Vector>> A, List<List<Vector>> B, int K) {
        ...
        double[] alpha = new double[K];; //initialize alpha
...
//the function that computes Vector x.
    public static Vector[] CentroidSelection(List<List<Vector>> A, List<List<Vector>> B, int K) {
        ...
        Vector X = ComputeVectorX(fixedA, fixedB, alpha, beta, L, K,5);
    ...
    public static Vector ComputeVectorX(double fixedA, double fixedB, double[] alpha, double[] beta, double[] dist, int K, double T ){
        ...
        for (int t=0; t<T; t++){
            ...
            for (int i=0; i<K; K++){
                double xi = ((1-gamma)*beta[i]*dist[i])/(gamma*alpha[i]+(1-gamma)*beta[i]);
                ArrayX[i] = xi;
                fAx = fAx + alpha[i]*xi*xi;
                fBx = fBx + beta[i]*(dist[i]-xi)*(dist[i]-xi);
                if (fAx == fBx){
                    break;
                } else{
                    if (fAx > fBx){
    ...
    public static void main(String[] args){
        ...
        System.out.println("Time to compute Standard Centers = ");
        System.out.println("Time to compute Fair Centers = ");
        System.out.println("Time to compute Objective with Standard Centers = ");
        System.out.println("Time to compute Objective with Fair Centers = ");
